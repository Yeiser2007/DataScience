...
def mostrar_resultados(exactitud, conf_matrix, metrics):
    ...
    canvas.draw()
    canvas.get_tk_widget().pack(side="right",fill="y", expand=False)
    ...
    # Precisión general
    precision_general = np.mean([values['precision'] for values in metrics.values()])
    precision_general_label = tk.Label(frame_result, text=f"Precisión general: {precision_general:.2f}", font=("Arial", 12))
    print(f"Precisión general: {precision_general:.2f}")
    precision_general_label.pack()

    # Media geométrica
    media_geometrica = np.sqrt(np.prod([values['precision'] for values in metrics.values()]))
    print(f"Media geométrica: {media_geometrica:.2f}")
    media_geometrica_label = tk.Label(frame_result, text=f"Media geométrica: {media_geometrica:.2f}", font=("Arial", 12))
    media_geometrica_label.pack()

    # Otros detalles relacionados al método
    # Puedes añadir más métricas o detalles según sea necesario aquí
...
def ejecutar_algoritmo_synthetic():
    ...
    if datos_entrenamiento:
        ...
        guardar_dataset_procesado(datos_balanceados, [dato[0] for dato in datos_balanceados], 'dataProcessed/dataset_procesado.csv')
